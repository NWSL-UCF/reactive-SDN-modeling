...
public class ReactiveForwarding {
    ...
    private class ReactivePacketProcessor implements PacketProcessor {
        ...
        public void process(PacketContext context) {
            ...
            log.info("Missed packet");

            long start_time = System.nanoTime();
//             Introduce a sleep timer to control processing rate to 2pkt/s 1/2=0.5
            LockSupport.parkNanos(TimeUnit.MILLISECONDS.toNanos(500));

            long end_time = System.nanoTime();
            double elapsed = (end_time - start_time);
            if (context.isHandled()) {
                elapsed = (end_time - start_time) / 1_000_000.0;
                log.info("elapsed 1st {} ms",elapsed);
                return;
            }
            ...
            elapsed = (end_time - start_time) / 1_000_000.0;
            log.info("elapsed 2nd {} ms",elapsed);
            if (ethPkt == null) {
                return;
            }
            ...
            if (isControlPacket(ethPkt)) {
                ...
                end_time = System.nanoTime();
                elapsed = (end_time - start_time)/ 1_000_000.0;
                log.info("elapsed 3rd {} ms",elapsed);
                return;
            }
            ...
            if (!ipv6Forwarding && isIpv6Multicast(ethPkt)) {
                ...
                end_time = System.nanoTime();
                elapsed = (end_time - start_time)/ 1_000_000.0;
                log.info("elapsed 4th {} ms",elapsed);
                return;
            }
            ...
            if (id.mac().isLldp()) {
                ...
                end_time = System.nanoTime();
                elapsed = end_time - start_time/ 1_000_000.0;
                log.info("elapsed 5th {} ms",elapsed);
                return;
            }
            ...
            if (ignoreIPv4Multicast && ethPkt.getEtherType() == Ethernet.TYPE_IPV4) {
                if (id.mac().isMulticast()) {
                    end_time = System.nanoTime();
                    elapsed = (end_time - start_time)/ 1_000_000.0;
                    log.info("elapsed 6th {} ms",elapsed);
                    return;
                }
            }
            ...
            if (dst == null) {
                ...
                end_time = System.nanoTime();
                elapsed = ((end_time - start_time))/ 1_000_000.0;
                log.info("elapsed 7th {} ms",elapsed);
                return;
            }
            ...
            if (pkt.receivedFrom().deviceId().equals(dst.location().deviceId())) {
                ...
                end_time = System.nanoTime();
                elapsed = (end_time - start_time)/ 1_000_000.0;
                log.info("elapsed 8th {} ms",elapsed);
                return;
            }
            ...
            if (paths.isEmpty()) {
                ...
                end_time = System.nanoTime();
                elapsed = (end_time - start_time)/ 1_000_000.0;
                log.info("elapsed 9th {} ms",elapsed);
                return;
            }
            ...
            if (path == null) {
                ...
                end_time = System.nanoTime();
                elapsed = (end_time - start_time)/ 1_000_000.0;
                log.info("elapsed 10 {} ms",elapsed);
                return;
            }
            ...
            end_time = System.nanoTime();
            elapsed = (end_time - start_time)/ 1_000_000.0;
            log.info("elapsed 11th ENDENDENDNED {} ms",elapsed);
